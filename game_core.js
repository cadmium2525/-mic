...
let realtimePendingOffer = [];  // 候補マスモン配列（常に6体）。選択済みの編成プリセット（PVP_RENTAL_STATE.selectedPreset）から生成する。相手の候補はマッチング成立まで非公開
...
// キーワード入力画面
// -----------------------------------------------------
// 自分の候補6体は選択済みの編成プリセット（PVP_RENTAL_STATE.selectedPreset）から
// マッチング処理開始時（startRealtimeMatching / startRandomMatching）に生成する。
// 相手の候補は、マッチングが成立するまでわからない。
function showRealtimeKeywordScreen(itemLoadout, battleType) {
    if (!PVP_RENTAL_STATE.selectedPreset || !isPvpPresetComplete(PVP_RENTAL_STATE.selectedPreset)) {
        showToast('先に使用する編成プリセットを選んでください。');
        if (typeof goToPvpPresetSelectFromEntry === 'function') goToPvpPresetSelectFromEntry();
        return;
    }

    realtimePendingOffer = [];
    realtimePendingType = battleType || 'team';
    realtimePendingItems = itemLoadout || [];

    const preview = document.getElementById('realtime-selected-masmon-preview');
    preview.innerHTML = '';
    preview.className = 'bg-[#2a1b15] border border-sky-900/50 rounded-xl p-2.5 text-center';

    const isTeam = realtimePendingType === 'team';
    const header = document.createElement('div');
    header.className = 'text-[10px] text-sky-300 font-bold';
    header.textContent = isTeam ? '🛡️⚔️🛡️ 団体戦（マッチング成立後に候補6体から3体を選出）' : '⚔️ 個人戦（マッチング成立後に候補6体から1体を選出）';
    preview.appendChild(header);

    const presetName = PVP_RENTAL_STATE.selectedPreset ? PVP_RENTAL_STATE.selectedPreset.name : null;
    const note = document.createElement('div');
    note.className = 'text-[9px] text-gray-500 mt-1';
    note.textContent = presetName
        ? `使用する編成: 「${presetName}」（相手の候補はマッチング成立まで見えません）`
        : 'モンスター候補はマッチングが成立するまでわかりません。';
...
function buildRealtimeMyPayload() {
    // 候補6体は、あらかじめ画面遷移時に選んだ自分の編成プリセット（PVP_RENTAL_STATE.selectedPreset）
    // からこのタイミングで生成する。既に生成済み（同じ合言葉での再送信等）の場合は同じ候補を使い回す。
    if (!realtimePendingOffer || realtimePendingOffer.length === 0) {
        realtimePendingOffer = generatePvpPresetOffer(PVP_RENTAL_STATE.selectedPreset) || [];
    }
    return {
        id: getMyPlayerId(),
        name: GAME_STATE.playerName || 'ブリーダー',
        battleType: realtimePendingType,
        // 候補6体をそのままマッチング相手にも公開する。実際に出す規定数（1体/3体）の選出は
        // マッチング成立後、双方の候補を見せ合った上で行う（selectedTeamとして別途書き込む）。
        offer: realtimePendingOffer.map(m => ({
            name: m.name,
            emoji: m.emoji,
...
// マッチング成立画面へ遷移
// -----------------------------------------------------
// マッチング成立後は、双方の候補6体を見せ合いながら実際の出場モンスターを選ぶ
// 「選出フェーズ」（pvp_rental.js の enterPvpPickPhase）へそのまま引き継ぐ。
// 相手の退出検知・選出完了の監視はそちらのリスナー（attachPvpPickListener）が担う。
function enterRealtimeMatchedScreen(roomData) {
    detachRealtimeListener();
    enterPvpPickPhase(roomData);
}
...
function resetRealtimeRoomState() {
    ...
    realtimeRandomQueueRoomKey = null;
    realtimeRandomQueueListener = null;
    if (typeof resetPvpPickState === 'function') resetPvpPickState();
}
