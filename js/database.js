...
const SKILLS_DB = {
    ...
    sunakake: { name: '砂かけ', aura: 'yellow', cost: 15, type: 'int', hitRate: 93, force: 0.6, gutsDown: 20, effect: 'hitdown_stack_3', desc: '砂をかけて視界と闘志を奪う。相手GUTS-20。さらに命中した場合、相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    tane_gun: { name: '種ガン', aura: 'green', cost: 20, type: 'int', hitRate: 87, force: 1.1, gutsDown: 10, effect: 'hitdown_stack_3', desc: '硬い種を弾丸のように撃ち出す基本技。相手GUTS-10。さらに命中した場合、相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    tane_machinegun: { name: '種マシンガン', aura: 'green', cost: 32, type: 'int', hitRate: 84, force: 1.4, gutsDown: 15, effect: 'hitdown_stack_3', desc: '種を連射して相手を蜂の巣にする。相手GUTS-15。さらに命中した場合、相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    tornado_attack: { name: '竜巻アタック', aura: 'green', cost: 45, type: 'pow', hitRate: 74, force: 3.0, gutsDown: 20, effect: 'hitdown_stack_3', desc: '体を回転させ竜巻を起こす豪快な大技。相手GUTS-20。さらに命中した場合、砂塵が舞い相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    daisharin: { name: '大車輪', aura: 'green', cost: 40, type: 'pow', hitRate: 74, force: 2.2, gutsDown: 15, effect: 'hitdown_stack_3', desc: '剣を大きく振り回す遠距離の大技。相手GUTS-15。さらに命中した場合、目眩ましとなり相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    nobiru_punch: { name: 'のびーるパンチ', aura: 'blue', cost: 36, type: 'pow', hitRate: 78, force: 2.0, gutsDown: 15, effect: 'hitdown_stack_3', desc: '腕を伸ばして遠くまで殴りつける。相手GUTS-15。さらに命中した場合、相手の視界を乱し命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    renzoku_harite: { name: '連続はり手', aura: null, cost: 27, type: 'pow', hitRate: 93, force: 0.68, gutsDown: 6, critBonus: 0.05, effect: 'hitdown_stack_3', hitCount: 2, desc: '両手による高速の張り手を2回叩き込む高命中技。相手GUTS-6。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    renzoku_kagizume: { name: '連続かぎづめ', aura: 'red', cost: 25, type: 'pow', hitRate: 79, force: 0.6, gutsDown: 26, critBonus: 0.08, effect: 'hitdown_stack_3', hitCount: 2, desc: '鋭い鉤爪で相手を2回切り裂く。相手GUTS-26。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    spin_cutter: { name: 'スピンカッター', aura: 'yellow', cost: 22, type: 'pow', hitRate: 78, force: 0.9, gutsDown: 3, critBonus: 0.12, effect: 'hitdown_stack_3', desc: '身を回転させ鋭い一撃を叩き込む。相手GUTS-3。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    double_shoda: { name: 'ダブルぽん拳', aura: 'blue', cost: 15, type: 'pow', hitRate: 80, force: 0.7, gutsDown: 13, critBonus: 0.09, effect: 'hitdown_stack_3', hitCount: 2, desc: '両手の掌底を2回叩き込む。相手GUTS-13。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    onitsume: { name: '鬼爪', aura: 'yellow', cost: 20, type: 'pow', hitRate: 77, force: 1.1, gutsDown: 7, critBonus: 0.13, effect: 'hitdown_stack_3', desc: '鋭く伸びた鬼の爪で相手を切り裂く。相手GUTS-7。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    combination: { name: 'コンビネーション', aura: 'yellow', cost: 55, type: 'pow', hitRate: 94, force: 0.64, gutsDown: 24, critBonus: 0.04, effect: 'hitdown_stack_3', hitCount: 2, desc: '緩急をつけた2回攻撃で相手を翻弄する高命中の大技。相手GUTS-24。さらに命中した場合、目が眩み相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    muchi: { name: 'ムチ', aura: 'blue', cost: 16, type: 'pow', hitRate: 85, force: 0.5, gutsDown: 3, critBonus: 0, effect: 'hitdown_stack_3', desc: '体を鞭のようにしならせて打ちつける高命中の基本技。相手GUTS-3。さらに命中した場合、鋭い一撃で相手の視界が乱れ命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    tobe_shinritsu_no_yaiba: { name: '翔べ震律の刃よ', aura: 'yellow', cost: 21, type: 'int', hitRate: 79, force: 0.5, gutsDown: 16, critBonus: 0.07, effect: 'hitdown_stack_3', desc: '震える法則を纏った不可視の刃を飛ばす。相手GUTS-16。さらに命中した場合、感覚を乱され相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    assault_arrow: { name: 'アサルトアロー', aura: 'yellow', cost: 33, type: 'pow', hitRate: 81, force: 1.75, gutsDown: 7, critBonus: 0.16, effect: 'hitdown_stack_3', desc: '矢の連射で相手を蜂の巣にする。相手GUTS-7。さらに命中した場合、視界を乱され相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    one_two: { name: 'ワンツー', aura: null, cost: 19, type: 'pow', hitRate: 69, force: 0.73, gutsDown: 3, critBonus: 0.04, effect: 'hitdown_stack_3', hitCount: 2, desc: '素早い両前脚による2回攻撃。相手GUTS-3。さらに命中した場合、目にもとまらぬ連撃で相手の命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    pixie_van: { name: 'バン', aura: 'red', cost: 34, type: 'int', hitRate: 75, force: 2.3, gutsDown: 25, critBonus: 0.16, effect: 'hitdown_stack_3', desc: '気合の声とともに放つ強烈な衝撃波。相手GUTS-25。さらに命中した場合、衝撃波の余波で相手の視界が乱れ命中率が10%低下する（最大3回まで累積、交代するまで持続）' },
    ...
    zan_stunner_blitz: { name: 'スタナーブリッツ', aura: 'yellow', cost: 26, type: 'int', hitRate: 87, force: 1.9, gutsDown: 25, critBonus: 0.08, effect: 'stun_debuff_once', desc: '電光のような一閃で相手の体勢を崩す。相手GUTS-25。さらに命中した場合、相手の命中率を10%、丈夫さを15%下げる（交代するまで持続・重複不可）' },
