...
(function () {
  ...
  function updateHotbarUI() {
    document.querySelectorAll('.hotbar-slot').forEach((el, i) => {
      el.classList.toggle('active', engine.player && engine.player.hotbarIndex === i);
    });
  }

  function onCraft(id) {
    if (!engine.player) return;
    Game.Crafting.craft(engine.player.inventory, id);
    updateHud();
  }
  ...
  window.addEventListener('keydown', (e) => {
    if (e.code === 'KeyC') {
      ...
      togglePause();
    } else if (uiState === 'craft' && /^Digit[1-8]$/.test(e.code)) {
