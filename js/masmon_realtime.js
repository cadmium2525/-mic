...
let realtimePendingOffer = [];  // マッチング前に抽選された候補マスモン配列（常に6体。実際の出場選出はマッチング成立後に行う）
...
// キーワード入力画面
// -----------------------------------------------------
function showRealtimeKeywordScreen(offer, itemLoadout, battleType) {
    realtimePendingOffer = offer;
    realtimePendingType = battleType || (offer.length > 1 ? 'team' : 'solo');
    realtimePendingItems = itemLoadout || [];

    const preview = document.getElementById('realtime-selected-masmon-preview');
    preview.innerHTML = '';
    preview.className = 'bg-[#2a1b15] border border-sky-900/50 rounded-xl p-2.5 space-y-2 max-h-48 overflow-y-auto';

    const isTeam = realtimePendingType === 'team';
    const header = document.createElement('div');
    header.className = 'text-[10px] text-sky-300 font-bold';
    header.textContent = isTeam ? `🛡️⚔️🛡️ 団体戦候補（${offer.length}体・マッチング成立後に3体を選出）` : `⚔️ 個人戦候補（${offer.length}体・マッチング成立後に1体を選出）`;
