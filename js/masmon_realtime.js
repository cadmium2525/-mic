...
let realtimePendingOffer = [];  // 候補マスモン配列（常に6体）。マッチング開始時に抽選され、内容はマッチング成立まで本人にも非公開のまま扱う
...
// キーワード入力画面
// -----------------------------------------------------
// 候補モンスター6体はここではまだ抽選しない（マッチング成立まで本人にも非公開にするため）。
// 実際の抽選は startRealtimeMatching / startRandomMatching でマッチング処理を開始する瞬間に行う。
function showRealtimeKeywordScreen(itemLoadout, battleType) {
    ...
    note.textContent = 'モンスター候補はマッチングが成立するまでわかりません。';
...
function buildRealtimeMyPayload() {
    // 候補6体はマッチング開始のこのタイミングで初めて抽選する（本人にも非公開のまま）。
    // 既に抽選済み（同じ合言葉での再送信等）の場合は同じ候補を使い回す。
    if (!realtimePendingOffer || realtimePendingOffer.length === 0) {
        realtimePendingOffer = generatePvpRentalOffer();
    }
    return {
        id: getMyPlayerId(),
