...
function enterRealtimeBattleScreen(state) {
    ...
    // PvPにはステージ選択は存在しない（デバッグモードの模擬戦専用機能）。前回デバッグバトルの
    // 設定が残っていた場合に備えて、念のためここで明示的に「闘技場」にリセットしておく。
    if (typeof setBattleStage === 'function') setBattleStage('arena');
...
function renderRealtimeTeamIcons(state) {
    ...
    const renderSide = (container, teamObj, isPartnerSide) => {
        ...
        if (existingSlots.length !== teamObj.units.length) {
            container.innerHTML = '';
        }
        ...
        teamObj.units.forEach((unit, idx) => {
            ...
            const identityKey = isFainted ? 'fainted' : `${unit.monsterBaseName}|${unit.emoji}|${!!unit.isAwakened}|${unit.aura || ''}|${isPartnerSide}`;
