...
function playChoRollinmochiMotion(side) {
    ...
    let anim;
    try {
        anim = animTarget.animate([
            { transform: 'translateX(0) scale(1) rotate(0deg)', borderRadius: '0%', offset: 0 },
            { transform: `translateX(${travel * 0.15}px) scale(0.75) rotate(180deg)`, borderRadius: '50%', offset: 0.22 },
            { transform: `translateX(${travel * 0.85}px) scale(0.8) rotate(900deg)`, borderRadius: '50%', offset: 0.72 },
            { transform: `translateX(${travel}px) scale(0.92) rotate(1080deg)`, borderRadius: '50%', offset: 0.88 },
            { transform: `translateX(${travel * 0.7}px) scale(1) rotate(1080deg)`, borderRadius: '0%', offset: 1 }
        ], { duration, easing: 'ease-in-out' });
    } catch (e) { /* 非対応環境ではスプライトは動かないが、下の衝撃エフェクトだけは再生される */ }
    if (anim) anim.onfinish = () => { animTarget.style.borderRadius = ''; };
...
function playMossamaMotion(side) {
    ...
    try {
        anim = animTarget.animate([
            { transform: 'translate(0,0) scale(1) rotate(0deg)', borderRadius: '0%', offset: 0 },
            { transform: 'translate(0,-60px) scale(1.05) rotate(0deg)', borderRadius: '0%', offset: 0.28 },       // ジャンプ
            { transform: `translate(${dx * 0.9}px,-70px) scale(0.85) rotate(360deg)`, borderRadius: '50%', offset: 0.55 }, // 空中で丸まりながら移動
            { transform: `translate(${dx * 0.95}px,-8px) scale(0.9) rotate(540deg)`, borderRadius: '50%', offset: 0.8 },   // 相手の上から急降下
            { transform: `translate(${dx * 0.7}px,0px) scale(1) rotate(560deg)`, borderRadius: '0%', offset: 1 }           // 着地して自陣へ戻る
        ], { duration, easing: 'ease-in-out' });
