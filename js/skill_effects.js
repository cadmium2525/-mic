...
const SKILL_EFFECT_TYPE = {
    ...
    chiretsuzan:'rock_earth', cho_atamatsuki:'punch_melee', cho_dash_giri:'slash', cho_kitsunebi:'fire_small', cho_mochihou:'petal', cho_monotaore:'punch_melee',
    ...
    zan_ohzantou:'slash', zan_rising_rave:'holy_light', zan_single_shot:'slash', zan_stunner_blitz:'electric', zoom_punch:'punch_melee', zoom_punch_nendoro:'punch_melee',
    zutsuki:'punch_melee',
};
...
const SKILL_EFFECT_CONFIGS = {
    ...
    holy_light:    { particles: ['✨'],          motion: 'beam',           count: 3, size: 22, duration: 500 },
        ...
        switch (config.motion) {
            ...
            case 'beam':
                ...
                keyframes = [
                    ...
                    { transform: `translate(${dx}px, ${dy}px) translate(-50%,-50%) scale(0.8)`, opacity: 0 }
                ];
                break;
            case 'slash':
            case 'impact':
...
function playSkillVisualEffect(skKey, side) {
    ...
    if (!effType) return;
    playSkillVisualEffectByType(effType, side);
}
...
function playSkillVisualEffectByType(effType, side) {
    const config = SKILL_EFFECT_CONFIGS[effType];
    if (!config) return;
